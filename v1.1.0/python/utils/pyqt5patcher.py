...
class PyQt5Patcher(PySide2Patcher):
    ...
    @classmethod
    def _patch_QTreeWidgetItem(cls, QtGui):
        """
        patch for QIcon from QTreeWidgetItem
        """
        original_QTreeWidgetItem = QtGui.QTreeWidgetItem
        class QTreeWidgetItem(original_QTreeWidgetItem):
            @classmethod
            def setIcon(self, int, icon):
                # item = QtGui.QTreeWidgetItem()
                if icon == None:
                    return super(QTreeWidgetItem, self).setIcon(0, QtGui.QIcon())
                return super(QTreeWidgetItem, self).setIcon(0, QtGui.QIcon(icon))

        QtGui.QTreeWidgetItem = QTreeWidgetItem

    @classmethod
    def _patch_QPixmapScreenGrab(cls,QtGui):
        original_QPixmap = QtGui.QPixmap
        class QPixmapScreenGrab(QtGui.QPixmap):
            @classmethod
            def grabWindow(cls, window_id, x, y, width, height):
                # This will only work for capturing one screen.  This would
                # ideally be changed in shotgun to call QScrren.grabWindow instead.
                screen = QtWidgets.QApplication.primaryScreen()
                return screen.grabWindow(window_id, x, y, width, height)
        QtGui.QPixmap = QPixmap

    @classmethod
    def _patch_QPainter(cls,QtGui):
        original_QPainter = QtGui.QPainter
        class QPainter(QtGui.QPainter):
            @classmethod
            def render(self,painter,target,source,flag):
                return super(QPainter, self).render(painter, target=QtCore.QPoint(0, 0), source=QtGui.QRegion(),flag=QtGui.QWidget.DrawChildren)
    ...
    def _patch_QPixmap(cls,QtGui):
        ...
        class QPixmap(QtGui.QPixmap):
            def grabWindow(self, window_id, x, y, width, height):
                # This will only work for capturing one screen.  This would
                # ideally be changed in shotgun to call QScreen.grabWindow instead.
                screen = QtWidgets.QApplication.primaryScreen()
                return screen.grabWindow(window_id, x, y, width, height)
        QtGui.QPixmap = QPixmap
    ...
    def patch(cls, QtCore, QtGui, QtWidgets):
        ...
        # cls._patch_QTreeWidgetItem(qt_gui_shim)
        # cls._patch_QPainter(qt_gui_shim)
        # cls._patch_QPixmapScreenGrab(qt_gui_shim)
